...
pub trait ConfigurationService: Send + Sync {
    /// 加载配置
    async fn load_config(&self, path: &str) -> std::result::Result<Configuration, crate::domain_errors::DomainError>;

    /// 保存配置
    async fn save_config(&self, config: &Configuration, path: &str) -> std::result::Result<(), crate::domain_errors::DomainError>;
