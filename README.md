...
### 🔒 智能安全扫描 (`gitai scan`)
- **Semgrep 集成**：使用业界领先的静态分析工具进行安全扫描
- **AI 增强分析**：AI 解读扫描结果，提供优先级排序和修复建议
- **自定义规则**：支持项目特定的安全规则和最佳实践检查
- **CI/CD 集成**：GitHub Actions 工作流自动化安全检查
- **多语言支持**：全面支持 Rust、Python、JavaScript、Java、Go 等语言
- **智能过滤**：按严重性级别过滤，专注于关键安全问题
...
gitai scan --severity=WARNING
gitai scan --severity=INFO
...
# 组合多个选项
gitai scan --detailed --ai --severity=WARNING --exclude="tests"
```
...
# 组合自定义规则和输出
gitai scan --rules="p/owasp-top-ten" --detailed --output=owasp-scan.json
```
...
# 生成详细报告（适合定期审计）
gitai scan --detailed --ai --output=weekly-security-$(date +%Y%m%d).json

# 快速安全检查
gitai scan --exclude="tests,docs,examples" --severity=ERROR
```
