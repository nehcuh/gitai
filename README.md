...
- **多格式输出**：默认 Markdown 格式，支持 JSON 格式，自动本地存储
...
#### 输出格式和分析
```bash
# 默认 Markdown 格式（自动生成文件名）
gitai scan

# 指定 JSON 格式
gitai scan --format=json --output security-report.json

# 指定 Markdown 格式（可选，默认）
gitai scan --format=markdown --output security-report.md

# 短格式选项
gitai scan -f json -o report.json
gitai scan -f md -o report.md

# 启用 AI 分析扫描结果
gitai scan --ai-analysis

# 组合多个选项
gitai scan --detailed --severity HIGH --exclude "tests/" --format=json --output report.json
```
