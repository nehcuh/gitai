...
use gitai::infrastructure::container::{ServiceContainer, ContainerError};
...
async fn test_singleton_behavior() {
    ...
    for i in 0..50 {
        ...
        handles.push(tokio::spawn(async move {
            let service = container_clone.resolve::<CalculatorService>().await.unwrap();
...
async fn test_concurrent_resolution_performance() {
    ...
    for _ in 0..1000 {
        let container_clone = container.clone();
        handles.push(tokio::spawn(async move {
            container_clone.resolve::<CalculatorService>().await.unwrap()
        }));
    }
...
async fn test_service_dependencies() {
    ...
    // 注册基础服务
    container.register(|_| Ok(LoggerService { 
        prefix: "TestApp".to_string() 
    }));
    
    // 注册依赖服务的工厂（简化版本，避免异步闭包复杂性）
    container.register(|_| {
        Ok(CalculatorService { multiplier: 10 })
    });
...
async fn test_error_handling() {
    ...
    // 注册一个总是失败的服务
    container.register(|_| Err(Box::new(std::io::Error::new(
...
async fn test_performance_benchmark() {
    ...
    println!("Cache hit rate: {:.2}%", container.get_cache_hit_rate() * 100.0);
...
async fn test_comprehensive_integration() {
    ...
    // 1. 注册多个不同类型的服务
    container.register(|_| Ok(CalculatorService { multiplier: 5 }));
    container.register(|_| Ok(LoggerService { prefix: "IntegrationTest".to_string() }));
    container.register(|_| Ok(TestService { id: 999 }));
    ...
    for i in 0..10 {
        let container_clone = container.clone();
        handles.push(tokio::spawn(async move {
            let calc = container_clone.resolve::<CalculatorService>().await.unwrap();
