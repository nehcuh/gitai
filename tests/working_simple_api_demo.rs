...
async fn demo_simple_transient() {
    ...
    // 使用直接的闭包语法
    container.register_transient(|_container| {
        Ok(SimpleService { value: 100 })
    }).await;
...
async fn demo_error_handling() {
    ...
    // 注册一个总是失败的服务
    container.register_singleton(|_container| -> Result<SimpleService, ContainerError> {
        Err(ContainerError::ServiceCreationFailed("配置错误".to_string()))
    }).await;
