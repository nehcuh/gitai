...
impl ServiceProvider for SimpleProvider {
    ...
    fn create(&self, _container: &ServiceContainer
...
async fn test_simple_api_works() {
    ...
    container.register_singleton(|_container| {
        Ok::<_, ContainerError>(TestService { value: 456 })
    }).await;
    println!("闭包语法注册完成");
...
async fn test_type_inference_issue() {
    ...
    let container = ServiceContainer::new();
    println!("尝试2: 显式类型标注...");
    let factory: fn() -> Result<TestService, ContainerError> = || {
        Ok(TestService { value: 222 })
    };
