...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   - 活跃作用域实例: {}", initial_stats.active_scoped_instances);
    ...
    println!("   瞬态服务创建次数: {}", after_transients.transient_creations);
    ...
    println!("   ❌ 单例缓存未命中: {}", final_stats.singleton_cache_misses);
    println!("   ⚡ 瞬态服务创建: {}", final_stats.transient_creations);
    println!("   🎯 循环依赖检测: {}", final_stats.circular_dependency_checks);
    
    // 服务创建分布
    let (singletons, transients, scoped) = final_stats.service_creation_distribution();
    println!("\n📊 服务创建分布:");
    println!("   单例: {}, 瞬态: {}, 作用域: {}", singletons, transients, scoped);
    ...
    println!("   活跃单例数量: {} (不受影响)", reset_stats.active_singletons);
