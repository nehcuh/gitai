...
fn test_error_from_std_types() {
    ...
    assert!(matches!(gitai_error, GitAIError::FileSystem(FileSystemError::Io(_))));
    
    // 测试从serde_json::Error转换
    let json_error = serde_json::from_str::<serde_json::Value>("invalid json").unwrap_err();
    let gitai_error: GitAIError = json_error.into();
    assert!(matches!(gitai_error, GitAIError::Parse(ParseError::Json(_))));
}

#[test]
fn test_error_context_enhancement() {
    // 测试错误上下文增强
    let result: std::result::Result<(), io::Error> = Err(io::Error::new(io::ErrorKind::PermissionDenied, "access denied"));
...
fn test_error_macros() {
    // 测试错误上下文宏
    let result: std::result::Result<(), io::Error> = Err(io::Error::new(io::ErrorKind::NotFound, "not found"));
...
fn test_display_implementations() {
    ...
    let errors = vec![
        ...
        GitAIError::Mcp(McpError::InvalidParameters("missing required param".to_string())),
    ];
