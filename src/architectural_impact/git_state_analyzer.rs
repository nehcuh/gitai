...
impl GitStateAnalyzer {
    ...
    /// 懒加载 TreeSitter 管理器
    async fn get_or_init_tree_sitter(&mut self) -> Result<&mut TreeSitterManager, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub async fn analyze_all_changed_files(
        &mut self,
        base_commit: Option<&str>
    ) -> Result<HashMap<String, (StructuralSummary, StructuralSummary)>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for file_path in &changed_files {
            ...
            match (
                ...
                (Err(before_err), Err(after_err)) => {
                    log::error!("无法分析文件 {} 的变更: before={}, after={}", 
                               file_path, before_err, after_err);
                }
            }
        }
    ...
    /// 分析 git diff 并返回架构影响
    pub async fn analyze_git_diff(&self, diff: &str) -> Result<ArchitecturalImpact, Box<dyn std::error::Error + Send + Sync>> {
            ...
            if line.starts_with("diff --git") {
                ...
                if content.contains("trait ") || content.contains("interface ") {
                    if let Some(interface_name) = extract_interface_name(content) {
                        ...
                        // 接口变更通常是破坏性的
                        breaking_changes.push(format!("接口变更: {} in {}", interface_name, current_file));
                    }
                }
            } else if line.starts_with("-") && !line.starts_with("---") {
                // 分析删除的行
                ...
                if content.contains("fn ") || content.contains("function") || content.contains("def ") {
                    if let Some(func_name) = extract_function_name(content) {
                        ...
                        // 函数删除是破坏性变更
                        breaking_changes.push(format!("函数删除: {} in {}", func_name, current_file));
                    }
                }
            }
        }
...
/// 从文件路径推断编程语言
pub fn infer_language_from_path(path: &str) -> Result<SupportedLanguage, Box<dyn std::error::Error + Send + Sync>> {
...
pub async fn analyze_file_architectural_impact(
    file_path: &str,
    base_commit: Option<&str>
) -> Result<crate::architectural_impact::ArchitecturalImpactAnalysis, Box<dyn std::error::Error + Send + Sync>> {
    let mut analyzer = GitStateAnalyzer::new_async().await?;
    
    let before_summary = analyzer.analyze_before_state(file_path, base_commit).await?;
...
mod tests {
    ...
    fn test_language_inference() {
        assert!(matches!(infer_language_from_path("test.rs"), Ok(SupportedLanguage::Rust)));
        assert!(matches!(infer_language_from_path("test.java"), Ok(SupportedLanguage::Java)));
        assert!(matches!(infer_language_from_path("test.py"), Ok(SupportedLanguage::Python)));
