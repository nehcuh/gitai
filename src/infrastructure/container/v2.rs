...
pub trait ServiceFactory: Send + Sync {
    ...
    ) -> Result<Arc<dyn Any + Send + Sync>, ContainerError>;
...
where
    F: Fn(&ServiceContainer) -> Result<T, Box<dyn std::error::Error + Send + Sync>>
        + Send
        + Sync
        + 'static,
    T: Send + Sync + 'static,
...
impl ServiceContainer {
    ...
    pub fn new() -> Self {
        Self {
            ...
            active_scopes: Arc::new(RwLock::new(Vec::new())),
            stats: Arc::new(InnerStats::default()),
        }
    }

    /// 内部：按生命周期注册工厂
    fn register_with<T, F>(&self, lifetime: ServiceLifetime, factory: F)
    where
        F: Fn(&ServiceContainer) -> Result<T, Box<dyn std::error::Error + Send + Sync>>
            + Send
            + Sync
        ...
        T: Send + Sync + 'static,
    {
        let factory = Box::new(FnServiceFactory::<F, T>::new(
            factory,
            std::any::type_name::<T>(),
        ));
    ...
    where
        ...
        T: Send + Sync + 'static,
    {
        self.register_with::<T, _>(ServiceLifetime::Singleton, factory);
    }
    ...
    where
        ...
        T: Send + Sync + 'static,
    {
        self.register_with::<T, _>(ServiceLifetime::Transient, move |_| Ok(factory()));
    }
    ...
    where
        ...
        T: Send + Sync + 'static,
    {
        self.register_with::<T, _>(ServiceLifetime::Singleton, move |c| Ok(factory(c)));
    }
    ...
    pub async fn resolve<T: Send + Sync + 'static>(&self) -> Result<Arc<T>, ContainerError> {
        ...
        let registration = match self.registrations.get(&type_id) {
            Some(reg) => reg,
            None => return Err(ContainerError::ServiceNotRegistered(type_id)),
        };
        ...
        match registration.lifetime {
            ServiceLifetime::Singleton => {
                ...
                let service = once_cell
                    .get_or_try_init(|| async { registration.value().factory.create(self) })
                    .await?;
            ...
            ServiceLifetime::Transient => {
                // 每次创建新实例，不参与缓存命中统计
                let service_any = registration.value().factory.create(self)?;
                let any_for_err = service_any.clone();
                match service_any.downcast::<T>() {
                    Ok(arc_t) => Ok(arc_t),
                    Err(_) => Err(ContainerError::TypeCastFailed {
                        expected: std::any::type_name::<T>().to_string(),
                        actual: format!("{:?}", any_for_err.type_id()),
                    }),
                }
            }
            ServiceLifetime::Scoped => {
                // 需要有活动作用域
                let (current_scope_id, _current_scope_name) = {
                    let guard = self.active_scopes.read().await;
                    match guard.last() {
                        ...
                        None => {
                            return Err(ContainerError::ScopeError {
                                ...
                                reason: "No active scope available".to_string(),
                            })
                        }
                    }
                };
                ...
                if let Some(inst) = scope_map.get(&type_id) {
                    ...
                    match cloned.downcast::<T>() {
                        ...
                        Err(_) => {
                            return Err(ContainerError::TypeCastFailed {
                                expected: std::any::type_name::<T>().to_string(),
                                actual: "scoped_cached_value".to_string(),
                            })
                        }
                    }
                }
                ...
                // 创建并缓存到当前作用域
                let created = registration.value().factory.create(self)?;
                let created_for_err = created.clone();
                let typed = match created.downcast::<T>() {
                    Ok(arc_t) => arc_t,
                    Err(_) => {
                        return Err(ContainerError::TypeCastFailed {
                            expected: std::any::type_name::<T>().to_string(),
                            actual: format!("{:?}", created_for_err.type_id()),
                        })
                    }
                };
                scope_map.insert(type_id, typed.clone());
                Ok(typed)
            }
        }
    }

    /// 开始一个无名作用域（默认名："default"）
    pub async fn begin_scope(&self) {
        self.begin_scope_named("default".to_string()).await;
    }

    /// 创建并激活命名作用域
    pub async fn begin_scope_named(&self, name: String) {
        let id = Uuid::new_v4();
        self.scopes.entry(id).or_default();
        let mut guard = self.active_scopes.write().await;
        guard.push((id, name));
    }

    /// 结束当前作用域
    pub async fn end_scope(&self) -> Result<(), ContainerError> {
        let popped = {
            let mut guard = self.active_scopes.write().await;
            guard.pop()
        };
        if let Some((id, _name)) = popped {
            ...
            Err(ContainerError::ScopeError {
                ...
                reason: "No active scope to end".to_string(),
            })
        }
    }
