    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ContainerError::CircularDependency {
                ...
                write!(
                    f,
                    ". Cycle at: {}. Resolution stack: {:?}",
                    cycle_point, resolution_stack
                )?;
                Ok(())
            }
            ...
            ContainerError::TypeCastFailed {
                ...
                write!(
                    f,
                    "Type cast failed: expected '{}', found '{}' in {}",
            ...
            ContainerError::ScopeError {
                ...
                write!(
                    f,
                    "Scope '{}' error during '{}': {} (current state: {:?})",
...
impl ContainerStats {
    ...
    pub fn service_creation_distribution(&self) -> (u64, u64, u64) {
        (
            self.singleton_cache_misses,
            self.transient_creations,
            self.scoped_creations,
        )
    }
...
struct ScopeManager {
    ...
    /// 作用域实例缓存 (作用域ID -> 服务实例)
    scope_instances:
        Arc<RwLock<HashMap<uuid::Uuid, Arc<RwLock<HashMap<TypeId, Arc<dyn Any + Send + Sync>>>>>>>,
}
...
impl ScopeManager {
    ...
    /// 激活作用域
    async fn activate_scope(&self, scope_id: uuid::Uuid) -> Result<(), ContainerError> {
        let mut scopes = self.scopes.write().await;
        let mut scope_stack = self.active_scope_stack.write().await;

        let scope_info = scopes
            .get_mut(&scope_id)
    ...
    /// 获取当前激活的作用域
    async fn current_scope(&self) -> Option<(uuid::Uuid, ScopeInfo)> {
        let scope_stack = self.active_scope_stack.read().await;
        let scopes = self.scopes.read().await;

        scope_stack
            .last()
            .and_then(|scope_id| scopes.get(scope_id).map(|info| (*scope_id, info.clone())))
    }

    /// 结束作用域
    async fn end_scope(&self, scope_id: uuid::Uuid) -> Result<(), ContainerError> {
        let mut scopes = self.scopes.write().await;
        let mut scope_stack = self.active_scope_stack.write().await;
        let mut instances = self.scope_instances.write().await;
        ...
        let parent_id_opt = {
            ...
            if scope_info.child_count > 0 {
                return Err(ContainerError::ScopeError {
                    ...
                    operation: "end_scope".to_string(),
                    reason: format!(
                        "Cannot end scope with {} active child scopes",
                        scope_info.child_count
                    ),
                    current_state: scope_info.state.clone(),
                });
            }
...
impl ServiceContainer {
    ...
    /// 解析服务
    pub async fn resolve<T: Send + Sync + Clone + 'static>(
        &self,
        ...
        // 检查循环依赖
        {
            ...
            if stack.contains(&type_id) {
                let resolution_stack: Vec<String> =
                    stack.iter().map(|id| format!("{:?}", id)).collect();
    ...
    /// 内部解析逻辑
    async fn resolve_internal<T: Send + Sync + Clone + 'static>(
        &self,
    ) -> Result<Arc<T>, ContainerError> {
        let type_id = TypeId::of::<T>();
        ...
        let registration = {
            ...
            registrations
                ...
                .ok_or_else(|| {
                    ...
                    let available_services: Vec<String> =
                        registrations.keys().map(|id| format!("{:?}", id)).collect();

                    // 简单的模糊匹配建议
                    let suggestion = if type_name.contains("Config")
                        && available_services.iter().any(|s| s.contains("Config"))
                    {
                        Some("Check available Config services".to_string())
                    } else if type_name.contains("Service")
                        && available_services.iter().any(|s| s.contains("Service"))
                    {
        ...
        match registration.lifetime {
            ServiceLifetime::Singleton => self.resolve_singleton::<T>(registration).await,
            ServiceLifetime::Transient => self.resolve_transient::<T>(registration).await,
            ServiceLifetime::Scoped => self.resolve_scoped::<T>(registration).await,
        }
    }
    ...
    async fn resolve_scoped<T: Send + Sync + Clone + 'static>(
        ...
        if !scope_info.is_valid() {
            return Err(ContainerError::ScopeError {
                ...
                operation: "resolve_scoped_service".to_string(),
                reason: format!(
                    "Cannot resolve service in scope with state {:?}",
                    scope_info.state
                ),
            ...
            if let Some(cached) = scoped.get(&type_id) {
                ...
                return match cached.clone().downcast::<T>() {
                    ...
                    Err(_) => {
                        ...
                        let actual_type = "Arc<dyn Any + Send + Sync>";
                        let context = format!(
                            "scoped service cache lookup for type {:?} in scope '{}'",
                            type_id, scope_info.name
                        );
                        let backtrace_hint = Some(
                            "Check that the service was registered with the correct type"
                                .to_string(),
                        );
    ...
    #[deprecated(
        since = "1.1.0",
        note = "请使用 register_singleton 或 register_transient 明确指定生命周期"
    )]
    ...
    pub async fn get_service<T: Send + Sync + Clone + 'static>(&self) -> Result<T, ContainerError> {
    ...
    pub async fn contains_service<T: 'static>(&self) -> bool {
        self.is_registered::<T>().await
    }

    /// 向后兼容：获取所有注册的服务类型名称
    #[deprecated(since = "1.1.0", note = "请使用 get_registered_types 方法")]
    pub async fn get_service_types(&self) -> Vec<String> {
    ...
    pub async fn get_container_status(&self) -> String {
    ...
    pub async fn get_cache_hit_percentage(&self) -> f64 {
...
mod tests {
    ...
    async fn test_enhanced_scope_functionality() {
        ...
        let scope_id = container
            .begin_scope_named("test_scope".to_string())
            .await
            .unwrap();
    ...
    async fn test_nested_scopes() {
        ...
        // 创建父作用域
        let parent_scope_id = container
            .begin_scope_named("parent_scope".to_string())
            .await
            .unwrap();
        ...
        // 创建子作用域
        let child_scope_id = container
            .begin_scope_named("child_scope".to_string())
            .await
            .unwrap();
        ...
        assert_eq!(
            Arc::as_ptr(&parent_service),
            Arc::as_ptr(&parent_service_again)
        );

        // 结束父作用域
        container
            .scope_manager
            .end_scope(parent_scope_id)
            .await
            .unwrap();
    ...
    async fn test_scope_error_handling() {
        ...
        // 测试在已结束的作用域中创建子作用域
        let scope_id = container
            .begin_scope_named("temp_scope".to_string())
            .await
            .unwrap();
        container.scope_manager.end_scope(scope_id).await.unwrap();
        ...
        let result = container
            .scope_manager
            .create_scope("child".to_string(), Some(scope_id))
            .await;
        assert!(result.is_err());
        match result {
            Err(ContainerError::ScopeError { reason, .. }) => {
        ...
        let parent_id = container
            .begin_scope_named("parent".to_string())
            .await
            .unwrap();
        let child_id = container
            .begin_scope_named("child".to_string())
            .await
            .unwrap();
