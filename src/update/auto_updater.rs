...
use crate::update::types::{UpdateResult, UpdateItem, RuleDownloadResult, PromptSetupResult};
...
impl AutoUpdater {
    pub fn new(config: Config) -> Self {
        ...
        Self { config, state_dir, notifier }
    }
    ...
    pub async fn update_scan_rules(&self) -> Result<UpdateItem, UpdateError> {
        ...
        Ok(UpdateItem {
            name: "扫描规则更新".to_string(),
            success: true,
            message: format!("规则已更新，共 {} 个文件，目录: {}", result.total_rules, rules_dir.display()),
        })
    }
    ...
    async fn update_gitai_version(&self) -> Result<UpdateItem, UpdateError> {
        ...
        if latest_version != current_version {
            // 显示更新提示
            println!("🎯 GitAI有新版本可用: {} -> {}", current_version, latest_version);
            println!("   运行 'cargo install gitai --force' 更新");
            
            // 发送新版本通知
            self.notifier.notify_new_version_available(current_version, &latest_version);
        }
        ...
        Ok(UpdateItem {
            name: "版本检查".to_string(),
            success: true,
            message: format!("当前版本: {}, 最新版本: {}", current_version, latest_version),
        })
    }
    
    async fn download_scan_rules(&self, target_dir: &std::path::Path) -> Result<RuleDownloadResult, UpdateError> {
        use std::io::Cursor;
        use flate2::read::GzDecoder;
        use tar::Archive;
        use walkdir::WalkDir;

        let url = std::env::var("GITAI_RULES_URL")
            .unwrap_or_else(|_| "https://github.com/opengrep/opengrep-rules/archive/refs/heads/main.tar.gz".to_string());
        ...
        self.retry_async(|| {
            ...
            async move {
                ...
                Ok(RuleDownloadResult { sources: vec![url], total_rules: count })
            }
        }, 3, "下载并解压扫描规则").await
    }
    ...
    fn normalize_rules_layout(dir: &std::path::Path) -> Result<(), UpdateError> {
        ...
        for entry in fs::read_dir(dir)? {
            let e = entry?;
            let ft = e.file_type()?;
            if ft.is_file() { has_files = true; break; }
            if ft.is_dir() { subdirs.push(e.path()); }
        }
        if has_files { return Ok(()); }
        if subdirs.len() != 1 { return Ok(()); }
        let inner = &subdirs[0];
        // 将 inner 的内容上提到 dir
    ...
    /// 异步重试机制
    async fn retry_async<F, Fut, T>(&self, operation: F, max_retries: usize, operation_name: &str) -> Result<T, UpdateError>
    where
        F: Fn() -> Fut,
        Fut: std::future::Future<Output = Result<T, UpdateError>>,
    {
            ...
            match operation().await {
                ...
                Err(e) => {
                    ...
                    if attempt < max_retries {
                        ...
                        println!("⚠️ {}第{}次尝试失败，{}秒后重试...", operation_name, attempt, delay.as_secs());
        ...
        Err(last_error.unwrap_or_else(|| UpdateError::Download(format!("{}重试次数耗尽", operation_name))))
    }
