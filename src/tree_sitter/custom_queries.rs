...
use crate::tree_sitter::{unified_analyzer::LanguageQueries, SupportedLanguage};
...
"#;
    ...
    /// 加载所有自定义查询
    pub fn load_all_custom_queries(
        &mut self,
        ...
        for entry in std::fs::read_dir(&self.user_queries_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("toml") {
                ...
                match self.load_custom_query_file(&path) {
                    Ok(config) => {
                        self.custom_queries
                            .insert(config.metadata.language.clone(), config.queries.clone());
                        loaded_count += 1;
                        log::info!(
                            "加载自定义查询: {} ({})",
    ...
    fn load_custom_query_file(
        &self,
        path: &Path,
    ) -> Result<CustomQueryConfig, Box<dyn std::error::Error + Send + Sync>> {
    ...
    fn merge_queries(
        &self,
        default: &LanguageQueries,
        custom: &LanguageQueries,
    ) -> LanguageQueries {
        LanguageQueries {
            ...
            call_query: custom
                .call_query
                .clone()
                .or_else(|| default.call_query.clone()),
        }
    }
