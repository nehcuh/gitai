...
use crate::tree_sitter::{
    custom_queries::CustomQueryManager, ClassInfo, CommentInfo, FunctionInfo, StructuralSummary,
    SupportedLanguage,
};
...
impl UnifiedAnalyzer {
    ...
    fn extract_calls(
        ...
        node: Node,
        source: &[u8],
    ) -> Result<Vec<crate::tree_sitter::FunctionCallInfo>, Box<dyn std::error::Error + Send + Sync>>
    {
        ...
        for m in matches {
            ...
            if !callee.is_empty() {
                calls.push(crate::tree_sitter::FunctionCallInfo {
                    callee,
                    line,
                    is_method,
                });
