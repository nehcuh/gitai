...
impl PerformanceCollector {
    ...
    pub fn record_call_success(&self, tool_name: &str, execution_time_ms: u64) {
        ...
        let stats = tool_stats.entry(tool_name.to_string()).or_insert(ToolStats {
            calls: 0,
            successful_calls: 0,
            failed_calls: 0,
...
impl From<McpError> for ServiceError {
    fn from(err: McpError) -> Self {
        ServiceError::Transport(std::io::Error::new(std::io::ErrorKind::Other, err.to_string()))
    }
}
...
impl From<std::io::Error> for McpError {
    fn from(err: std::io::Error) -> Self {
        match err.kind() {
            std::io::ErrorKind::NotFound => McpError::FileOperationError(format!("File not found: {}", err)),
            std::io::ErrorKind::PermissionDenied => McpError::PermissionError(format!("Permission denied: {}", err)),
...
pub trait GitAiMcpService: Send + Sync {
    ...
    /// 处理工具调用
    async fn handle_tool_call(&self, name: &str, arguments: serde_json::Value) -> McpResult<serde_json::Value>;
}
...
impl GitAiMcpManager {
    ...
    pub fn new(config: crate::config::Config) -> Self {
        ...
        let mut services: std::collections::HashMap<String, Box<dyn GitAiMcpService + Send + Sync>> = std::collections::HashMap::new();
            ...
            if mcp_config.enabled {
                ...
                for service_name in &mcp_config.services.enabled {
                    ...
                    match service_name.as_str() {
                        "review" => {
                            match services::ReviewService::new(config.clone()) {
                                Ok(service) => {
                                    services.insert("review".to_string(), Box::new(service) as Box<dyn GitAiMcpService + Send + Sync>);
                        ...
                        "commit" => {
                            match services::CommitService::new(config.clone()) {
                                Ok(service) => {
                                    services.insert("commit".to_string(), Box::new(service) as Box<dyn GitAiMcpService + Send + Sync>);
                        ...
                        "scan" => {
                            match services::ScanService::new(config.clone()) {
                                Ok(service) => {
                                    services.insert("scan".to_string(), Box::new(service) as Box<dyn GitAiMcpService + Send + Sync>);
                                    info!("✅ 服务 'scan' 初始化成功");
                                }
                                Err(e) => {
                        ...
                        "analysis" => {
                            match services::AnalysisService::new(config.clone()) {
                                Ok(service) => {
                                    services.insert("analysis".to_string(), Box::new(service) as Box<dyn GitAiMcpService + Send + Sync>);
                ...
                info!("🎯 MCP 服务管理器初始化完成，共注册 {} 个服务", services.len());
            } else {
                info!("ℹ️  MCP 服务已禁用");
            }
        } else {
    ...
    /// 处理工具调用
    pub async fn handle_tool_call(&self, tool_name: &str, arguments: serde_json::Value) -> McpResult<serde_json::Value> {
        debug!("🔧 处理工具调用: {}", tool_name);
        debug!("📋 工具参数: {}", serde_json::to_string_pretty(&arguments).unwrap_or_default());
        ...
        for service in self.services.values() {
            ...
            if tools.iter().any(|tool| tool.name == tool_name) {
                ...
                match &result {
                    Ok(_) => {
                        self.performance_collector.record_call_success(tool_name, duration_ms);
                        info!("✅ 工具调用成功: {} (耗时: {:?})", tool_name, duration);
                    }
                    Err(e) => {
                        self.performance_collector.record_call_failure(tool_name, duration_ms);
                        warn!("❌ 工具调用失败: {} (耗时: {:?}, 错误: {})", tool_name, duration, e);
                    }
                }
        ...
        Err(invalid_parameters_error(format!("Unknown tool: {}", tool_name)))
    }
