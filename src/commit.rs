...
pub async fn execute_commit_with_result(
    ...
    let commit_hash =
        execute_git_operations_with_result(config, &commit_message, &commit_config).await?;
    ...
    details.insert(
        ...
        commit_config.tree_sitter.to_string(),
    );
...
async fn generate_commit_message(
    ...
    let tree_sitter_summary = if commit_config.tree_sitter {
        perform_structural_analysis(diff)
            .await?
...
async fn build_commit_prompt_fallback(
    config: &Config,
    diff: &str,
    issues: &[Issue],
    commit_config: &CommitConfig,
...
async fn perform_structural_analysis(
    ...
    match TreeSitterManager::new().await {
        Ok(mut manager) => match manager.analyze_structure(&code_content, supported_lang) {
            Ok(summary) => {
                println!("  ✅ 结构分析完成");
...
async fn perform_review_with_result(
    ...
    _diff: &str,
    issues: &[Issue],
) -> Result<Option<ReviewResults>, Box<dyn std::error::Error + Send + Sync>> {
    // 创建评审配置
...
/// 执行Git操作并返回提交哈希
async fn execute_git_operations_with_result(
    _config: &Config,
    commit_message: &str,
    commit_config: &CommitConfig,
