...
use super::types::{ReviewConfig, ReviewCache};
...
/// 检查缓存
pub fn check_cache(cache_key: &str) -> Result<Option<String>, Box<dyn std::error::Error + Send + Sync>> {
...
/// 保存缓存
pub fn save_cache(cache_key: &str, result: &str, language: &Option<String>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let cache_dir = dirs::home_dir()
        .unwrap_or_else(|| std::path::PathBuf::from("."))
        .join(".cache")
        .join("gitai")
