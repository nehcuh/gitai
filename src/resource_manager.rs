...
impl ResourceManager {
    ...
    async fn download_rules(&self) -> Result<()> {
        ...
        info!(
            "Downloading OpenGrep rules from {}",
            self.config.sources.rules_url
        );

        // Try primary source first
        if let Err(e) = self
            .download_from_git(&self.config.sources.rules_url, &rules_dir)
            .await
        {
            ...
            for fallback in &self.config.sources.fallback_sources {
                ...
                if self
                    .download_from_git(&fallback_url, &rules_dir)
                    .await
                    .is_ok()
                {
        ...
        // Update metadata
        self.update_metadata(&rules_dir, &self.config.sources.rules_url)
            .await?;
    ...
    async fn download_grammars(&self, language: &str) -> Result<()> {
        ...
        info!(
            "Downloading Tree-sitter grammar for {} from {}",
            language, url
        );
    ...
    async fn download_from_git(&self, repo_url: &str, target_dir: &Path) -> Result<()> {
        ...
        if repo_url.contains("github.com") {
            ...
            let archive_url = repo_url.replace(".git", "/archive/refs/heads/main.zip");
    ...
    async fn should_update(&self, resource_dir: &Path) -> Result<bool> {
        ...
        let metadata: ResourceMetadata = serde_json::from_str(&fs::read_to_string(metadata_file)?)?;
...
pub fn load_resource_config(config_path: &Path) -> Result<ResourceConfig> {
    ...
    let default_sources = toml::Value::Table({
        ...
        table.insert(
            "update_check_interval".to_string(),
    ...
    let default_cache = toml::Value::Table({
        ...
        table.insert(
            "path".to_string(),
            toml::Value::String("~/.cache/gitai".to_string()),
        );
        table.insert(
            "max_size".to_string(),
