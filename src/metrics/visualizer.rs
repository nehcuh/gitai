...
use super::{QualitySnapshot, Significance, Trend, TrendAnalysis};
use chrono::Utc;
use std::fmt::Write;
...
impl TrendVisualizer {
    ...
    fn write_executive_summary(
        ...
        writeln!(
            report,
            "**整体趋势**: {} {:?}",
            trend_emoji, analysis.overall_trend
        )?;
        writeln!(report)?;

        // 统计改善和恶化的指标
        let improving = analysis
            .metric_trends
            .values()
            .filter(|t| matches!(t.trend, Trend::Improving))
            .count();
        let degrading = analysis
            .metric_trends
            .values()
    ...
    fn write_key_findings(
        ...
        for finding in &analysis.key_findings {
            ...
            writeln!(
                report,
                "- **检测时间**: {}",
                finding.detected_at.format("%Y-%m-%d %H:%M")
            )?;
            writeln!(report)?;
        }
    ...
    fn write_trend_chart(
        ...
        for i in (0..=chart_height).rev() {
            ...
            for j in 0..chart_width {
                let snapshot_idx = j * snapshots.len() / chart_width;
                let value = snapshots[snapshot_idx]
                    .complexity_metrics
    ...
    fn write_detailed_data_table(
        ...
        writeln!(
            report,
            "| 日期 | Commit | LOC | 模块数 | 循环依赖 | 债务分 | 复杂度 | API稳定性 |"
        )?;
        writeln!(
            report,
            "|------|--------|-----|--------|----------|--------|--------|-----------|"
