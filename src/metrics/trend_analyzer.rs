...
use super::{
    QualitySnapshot, TrendAnalysis, Trend, MetricTrend, Significance,
    Finding, TimeRange, Predictions
};
...
impl<'a> TrendAnalyzer<'a> {
    ...
    /// 执行趋势分析
    pub fn analyze(&self, days_back: Option<i64>) -> Result<TrendAnalysis, Box<dyn std::error::Error + Send + Sync>> {
        // 过滤时间范围内的快照
        ...
        let time_range = TimeRange {
            start: filtered_snapshots.first().unwrap().timestamp,
            end: filtered_snapshots.last().unwrap().timestamp,
    ...
    /// 分析指标趋势
    fn analyze_metric_trends(&self, snapshots: &[&QualitySnapshot]) -> Result<HashMap<String, MetricTrend>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(debt_trend) = metric_trends.get("technical_debt") {
            if debt_trend.change_rate > 30.0 {
                findings.push(Finding {
                    ...
                    description: format!(
                        "技术债务增长了 {:.1}%，从 {:.1} 增加到 {:.1}",
                        debt_trend.change_rate,
                        debt_trend.previous_value,
                        debt_trend.current_value
                    ),
                    ...
                    match metric_name.as_str() {
                        "complexity" => {
                            recommendations.push(
                                "建议进行代码重构，降低复杂度。考虑拆分大函数和复杂逻辑".to_string()
                            );
                        }
                        "technical_debt" => {
                            recommendations.push(
            ...
            if finding.significance as i32 >= Significance::High as i32 {
                match finding.metric.as_str() {
                    "circular_dependencies" => {
                        recommendations.push(
                            "紧急：需要解决循环依赖问题，这会影响系统的可维护性和测试性".to_string()
                        );
                    }
                    _ => {}
                }
    ...
    fn generate_predictions(
        ...
        if let Some(trend) = debt_trend {
            if trend.change_rate > 10.0 {
                // 简单线性预测：假设按当前速率增长
                let days_to_critical = ((100.0 - trend.current_value) / (trend.change_rate / 30.0)) as i64;
                if days_to_critical > 0 && days_to_critical < 365 {
                    predictions.debt_critical_date = Some(Utc::now() + Duration::days(days_to_critical));
                }
            }
        }
    ...
    pub fn detect_anomalies(&self) -> Vec<(DateTime<Utc>, String)> {
        ...
        for i in 2..self.snapshots.len() {
            ...
            if (current.technical_debt.debt_score - avg_debt).abs() > avg_debt * 0.5 {
                anomalies.push((
                    current.timestamp,
                    format!("技术债务异常: {:.1} (平均: {:.1})", 
                           current.technical_debt.debt_score, avg_debt)
                ));
            }
            
            // 检查复杂度异常
            let avg_complexity: f64 = window.iter()
                .map(|s| s.complexity_metrics.avg_cyclomatic_complexity)
                .sum::<f64>() / window.len() as f64;
            
            if (current.complexity_metrics.avg_cyclomatic_complexity - avg_complexity).abs() > avg_complexity * 0.4 {
                anomalies.push((
                    current.timestamp,
                    format!("复杂度异常: {:.1} (平均: {:.1})", 
...
mod tests {
    ...
    fn create_test_snapshot(debt: f64, complexity: f64, timestamp: DateTime<Utc>) -> QualitySnapshot {
        QualitySnapshot {
            timestamp,
    ...
    fn test_anomaly_detection() {
        ...
        assert!(anomalies.iter().any(|(_, desc)| desc.contains("技术债务异常")));
    }
}
