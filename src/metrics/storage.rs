...
pub fn load_all_branches_snapshots(
    ...
    for entry in std::fs::read_dir(storage_path)? {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
            if let Some(branch_name) = path.file_stem()
                .and_then(|s| s.to_str())
...
pub fn export_to_csv(
    ...
    for snapshot in snapshots {
        writer.write_record(&[
            ...
            snapshot.architecture_metrics.circular_dependencies.to_string(),
            format!("{:.2}", snapshot.architecture_metrics.coupling_score),
            format!("{:.2}", snapshot.complexity_metrics.avg_cyclomatic_complexity),
            snapshot.complexity_metrics.max_cyclomatic_complexity.to_string(),
