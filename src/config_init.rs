...
impl ConfigInitializer {
        ...
        if !config_path.exists() {
            ...
            if self.offline_mode {
                ...
                if let Err(e) = self.try_download_default_config(&config_path).await {
                    warn!("Failed to download default config: {}, using embedded template", e);
                    self.copy_default_config(&config_path)?;
                }
            }
        } else {
    ...
    async fn try_download_default_config(&self, target: &Path) -> Result<()> {
        let default_url = "https://raw.githubusercontent.com/nehcuh/gitai/main/assets/config.enhanced.toml";
            ...
            if !config.get("version").is_some() {
                ...
                if let Some(table) = config.as_table_mut() {
                    ...
                    if !table.contains_key("sources") {
                        ...
                        sources.insert("rules_url".to_string(),
                            toml::Value::String("https://github.com/nehcuh/gitai-rules.git".to_string()));
                        sources.insert("tree_sitter_url".to_string(),
                    ...
                    if !table.contains_key("cache") {
                        ...
                        cache.insert("max_size".to_string(), toml::Value::String("1GB".to_string()));
...
mod tests {
    ...
    async fn test_config_initialization() {
        ...
        assert!(initializer.config_dir.join("prompts/commit-generator.md").exists());
    }
}
