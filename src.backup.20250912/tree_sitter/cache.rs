...
impl TreeSitterCache {
    ...
    pub fn new(
        ...
        let final_capacity = if capacity == 0 { 100 } else { capacity };
        // SAFETY: final_capacity is guaranteed to be > 0 by construction above
        let capacity_nz = unsafe { NonZeroUsize::new_unchecked(final_capacity) };

        Ok(Self {
            memory_cache: Arc::new(Mutex::new(LruCache::new(capacity_nz))),
            cache_dir,
            max_age_seconds,
