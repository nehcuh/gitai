...
impl ServiceRegistry {
    ...
    fn dfs_detect_cycle(
        ...
        if let Some(cycle_start) = path.iter().position(|s| s == current_id) {
            // 发现循环
    ...
    pub async fn get_startup_order(&self) -> McpResult<Vec<String>> {
        ...
        for (id, metadata) in services.iter() {
            for dep in &metadata.dependencies {
                if let Some(dep_metadata) = services.values().find(|m| m.name == dep.service_name) {
                    graph.entry(dep_metadata.id.clone()).or_default().push(id.clone());
                    let degree = in_degree.entry(id.clone()).or_insert(0);
                    *degree += 1;
                }
            }
        }
                ...
                for neighbor in neighbors {
                    if let Some(degree) = in_degree.get_mut(neighbor) {
                        *degree -= 1;
                        ...
                        return Err(McpError::ConfigurationError(format!("Inconsistency: missing in_degree for service {neighbor}")));
                    }
                }
            }
        }
